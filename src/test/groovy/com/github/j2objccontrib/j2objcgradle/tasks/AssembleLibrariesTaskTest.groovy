    ...
    void testAssembleLibraries() {
        ...
        AssembleLibrariesTask j2objcAssembleLibraries =
                (AssembleLibrariesTask) proj.tasks.create(name: 'j2objcAL', type: AssembleLibrariesTask) {
                    buildType = 'Debug'
                    libDir = proj.file("$proj.buildDir/binaries/$proj.name-j2objcStaticLibrary")
                    packedLibDir = proj.file("$proj.buildDir/packedBinaries/$proj.name-j2objcStaticLibrary")
                }
